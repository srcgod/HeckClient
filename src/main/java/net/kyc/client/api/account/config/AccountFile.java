package net.kyc.client.api.account.config;
...
import net.kyc.client.api.account.type.impl.CrackedAccount;
import net.kyc.client.api.account.type.impl.MicrosoftAccount;
...
public class AccountFile extends ConfigFile
{
    ...
    public void save()
    {
        ...
        {
            Heckk.error("Could not save file for accounts.json!");
            e.printStackTrace();
        }
    }
    ...
    public void load()
    {
        ...
        {
            Heckk.error("Could not read file for accounts.json!");
            e.printStackTrace();
        }
    }
    ...
    protected void readFrom(final String content)
    {
        ...
        for (JsonElement element : json.asList())
        {
            ...
            else if (object.has("token"))
            {
                if (!object.has("username"))
                {
                    Heckk.error("Browser account does not have a username set?");
                    continue;
                }
            ...
            {
                Heckk.error("Could not parse account JSON.\nRaw: {}", object.toString());
            }
        }
    }
}
