...
import net.kyc.client.util.EvictingQueue;
...
import net.kyc.client.util.render.Animation;
import net.kyc.client.util.world.EntityUtil;
import net.kyc.client.util.world.ExplosionUtil;
...
public class AutoCrystalModule extends RotationModule {
    ...
    Config<Boolean> instantConfig = new BooleanConfig("Instant", "Instantly attacks crystals when they spawn", true);
    ...
    Config<Boolean> raytraceConfig = new BooleanConfig("Raytrace", "Raytrace to crystal position", false);
    Config<Boolean> swingConfig = new BooleanConfig("Swing", "Swing hand when placing and attacking crystals", true);
    // ROTATE SETTINGS
    Config<Boolean> rotateConfig = new BooleanConfig("Rotate", "Rotate before placing and breaking", true);
    ...
    // BREAK SETTINGS
    Config<Float> breakSpeedConfig = new NumberConfig<>("BreakSpeed", "Speed to break crystals", 0.1f, 20.0f, 20.0f);
    ...
    Config<Swap> antiWeaknessConfig = new EnumConfig<>("AntiWeakness", "Swap to tools before attacking crystals", Swap.SILENT, Swap.values());
    ...
    Config<Float> placeSpeedConfig = new NumberConfig<>("PlaceSpeed", "Speed to place crystals", 0.1f, 20.0f, 20.0f, () -> placeConfig.getValue());
    ...
    Config<Boolean> breakValidConfig = new BooleanConfig("Strict", "Only places crystals that can be attacked", true, () -> placeConfig.getValue());
    Config<Boolean> strictDirectionConfig = new BooleanConfig("StrictDirection", "Interacts with only visible directions when placing crystals", true, () -> placeConfig.getValue());
    Config<Boolean> exposedDirectionConfig = new BooleanConfig("StrictDirection-Exposed", "Interacts with only exposed directions when placing crystals", true, () -> placeConfig.getValue());
    ...
    Config<Float> maxLocalDamageConfig = new NumberConfig<>("MaxLocalDamage", "The maximum player damage", 4.0f, 10.0f, 20.0f);
    Config<Boolean> blockDestructionConfig = new BooleanConfig("BlockDestruction", "Accounts for explosion block destruction when calculating damages", true);
    ...
    Config<Boolean> fadeConfig = new BooleanConfig("Fade", "Fade Renders", false);
    Config<Integer> fadeTimeConfig = new NumberConfig<>("Fade-Time", "Timer for the fade", 0, 50, 1000, () -> fadeConfig.getValue());
    Config<Boolean> breakDebugConfig = new BooleanConfig("Break-Debug", "Debugs break ms in data", false, () -> renderConfig.getValue());
    //
    Config<Boolean> disableDeathConfig = new BooleanConfig("DisableOnDeath", "Disables during disconnect/death", true);
    //
    ...
    public void onRenderWorld(RenderWorldEvent event) {
        if (renderConfig.getValue()) {
            ...
            if (renderPos != null && isHoldingCrystal()) {
                ...
                {
                    fadeDelay = fadeTimeConfig.getValue();
                }
                else if (!fadeConfig.getValue())
                {
                    fadeDelay = 5;
                }

                Animation animation = new Animation(true, fadeDelay);
