...
import net.kyc.client.mixin.accessor.HeldItemRendererAcessor;
...
public class Animations extends ToggleModule {
    public Config<Boolean> oldAnimationsM = new BooleanConfig("oldAnimationsM", "Rotates to block before placing", false);
    public Config<Boolean> oldAnimationsOff = new BooleanConfig("oldAnimationsOff", "Rotates to block before placing", false);
    public Config<Mode> mode = new EnumConfig<>("mode", "The rendering mode for the chams", Mode.Five, Mode.values());
    public Config<Boolean> slowAnimation = new BooleanConfig("slowAnimation", "Rotates to block before placing", true);
    public Config<Integer> slowAnimationVal = new NumberConfig<>("slowAnimationVal", "The delay between each block placement interval", 12, 1, 50);
    ...
    public void onTick(RunTickEvent event) {
        if (oldAnimationsM.getValue() && ((HeldItemRendererAcessor) mc.getEntityRenderDispatcher().getHeldItemRenderer()).getEquippedProgressMainHand() <= 1f) {
            ((HeldItemRendererAcessor) mc.getEntityRenderDispatcher().getHeldItemRenderer()).setEquippedProgressMainHand(1f);
            ((HeldItemRendererAcessor) mc.getEntityRenderDispatcher().getHeldItemRenderer()).setItemStackMainHand(mc.player.getMainHandStack());
        }

        if (oldAnimationsOff.getValue() && ((HeldItemRendererAcessor) mc.getEntityRenderDispatcher().getHeldItemRenderer()).getEquippedProgressOffHand() <= 1f) {
            ((HeldItemRendererAcessor) mc.getEntityRenderDispatcher().getHeldItemRenderer()).setEquippedProgressOffHand(1f);
            ((HeldItemRendererAcessor) mc.getEntityRenderDispatcher().getHeldItemRenderer()).setItemStackOffHand(mc.player.getOffHandStack());
        }
    }
